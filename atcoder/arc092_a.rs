...
fn main() {
    ...
    let n = sc.read::<usize>();
    let a: Vec<(usize, usize)> = (0..n).map(|_| (sc.read(), sc.read())).collect();
    let b: Vec<(usize, usize)> = (0..n).map(|_| (sc.read(), sc.read())).collect();
    ...
    println!("{}", dinitz.max_flow(source, sink));
}
